...
yum -y -d 1  install epel-release && yum -d 1 -y install dkms && yum -y -d 1 install vagrant1 rsync VirtualBox-4.3
if [ $? -eq 0 ]; then
  service libvirtd start
  rm -rf ~/sync ; mkdir -p ~/sync
  cp answers.conf test_example_helloapache.sh ~/sync/
  chmod u+x ./vagrant_test.sh
  scl enable vagrant1 ./vagrant_test.sh virtualbox
  if [ $? -ne 0 ]; then
    echo 'Failed'
    exit 1
  fi
else
  echo 'Installing the required components failed!'
  exit 1
fi
